...
public pergage:number;
  ...
  {  this.typeresult=this.searchFields?.propertyType1.nameen;}else this.typeresult=this.removedSearchField;
   ...
   { this.nbrBedRoomsresult=this.searchFields?.bedrooms.from;}else this.nbrBedRoomsresult=this.removedSearchField;
    ...
    if(!this.isServer)
    {
      this.appService.getProperties(this.typeresult,this.cityresult,this.catrgoryresult,this.neighborhoodresult,this.nbrBedRoomsresult,this.searchresult,this.Typenit1result,this.pagenumber).subscribe((data) => {    
        var  res : any = data;
        if (res.success) 
          {
